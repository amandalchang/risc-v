    ...
    jal   x13, 0x2a             # x13 = return addr 0x38
    ...
    sb    x17, -4(x24)          # store to 0x1000 - 4 = 0xFFC
    sb    x17, -3(x24)          # store to 0xFD
    sb    x17, -2(x24)          # store to 0xFE
    sb    x17, -1(x24)          # store to 0xFF
    lw    x18, -4(x24)          # load word from 0xFFC
    lw    x19, -12(x24)         # load word from 0x100 - 12 = 0xF4
    lh    x20, -4(x24)          # load halfword from 0xFFC
    lhu   x21, -4(x24)          # load halfword un-signed
    lb    x22, -4(x24)          # load byte from 0xFFC (signed)
    lbu   x23, -4(x24)          # load byte from 0xFFC (unsigned)
