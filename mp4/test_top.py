...
async def test_cpu(dut):
    ...
    for j in range(24):
        ...
        try:
            print(f"src a: {hex(dut.src_a.value.integer)}")
        except:
            print(f"src a: {dut.src_a.value}")

        # src b (hex if possible)
        try:
            print(f"src b: {hex(dut.src_b.value.integer)}")
        except:
        ...
        # alu result (you printed alu_result in the original)
        try:
            print(f"alu result: {hex(dut.alu_result.value.integer)}")
        except:
        ...
        # pc
        try:
            print(f"{hex(dut.pc.value.integer)} pc")
        except:
            print(f"{dut.pc.value} pc")

        # old pc
        try:
            print(f"{hex(dut.old_pc.value.integer)} old pc")
        except:
            print(f"{dut.old_pc.value} old pc")

        # pc next
        try:
            print(f"{hex(dut.pc_next.value.integer)} pc next")
        except:
            print(f"{dut.pc_next.value} pc next")

        # imem address
        try:
            print(f"imem_address: {hex(dut.imem_address.value.integer)}")
        except:
        ...
        # result (WD3)
        try:
            print(f"result (aka WD3): {hex(dut.result.value.integer)}")
        except:
